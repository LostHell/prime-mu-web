...
type ItemTooltipProviderProps = {
  children: React.ReactNode;
  delayDuration?: number;
};

type ItemTooltipRootProps = {
  children: React.ReactNode;
  delayDuration?: number;
};

type ItemTooltipTriggerProps = {
  asChild?: boolean;
...
const ItemTooltipProviderContext = React.createContext<ProviderValue | null>(
  null,
);

function ItemTooltipProvider({
  children,
  delayDuration = 0,
}: ItemTooltipProviderProps) {
  const value = React.useMemo(() => ({ delayDuration }), [delayDuration]);

  return (
    <ItemTooltipProviderContext.Provider value={value}>
      {children}
    </ItemTooltipProviderContext.Provider>
  );
}
...
const ItemTooltipRootContext = React.createContext<RootValue | null>(null);

function useRoot() {
  const ctx = React.useContext(ItemTooltipRootContext);
  if (!ctx)
    throw new Error(
      "ItemTooltip components must be used within <ItemTooltip />",
    );
  return ctx;
}
...
function ItemTooltip({ children, delayDuration }: ItemTooltipRootProps) {
  const provider = React.useContext(ItemTooltipProviderContext);
  ...
  return (
    <ItemTooltipRootContext.Provider value={value}>
      {children}
    </ItemTooltipRootContext.Provider>
  );
}

function ItemTooltipTrigger({
  asChild,
  children,
  onMouseEnter,
  onMouseLeave,
